...
import (
	...
	"github.com/certusone/wormhole/node/pkg/watchers/tron"
...
func init() {
	...
	tronRPC = node.RegisterFlagWithValidationOrFail(NodeCmd, "tronRPC", "tron RPC URL", "http://localhost:50051", []string{"http"})
	...
	*polygonSepoliaContract = checkEvmArgs(logger, *polygonSepoliaRPC, *polygonSepoliaContract, vaa.ChainIDPolygonSepolia)
	...
	if shouldStart(tronRPC) {
		wc := &tron.WatcherConfig{
			...
			GuardianSetUpdateChain: true,
			CcqBackfillCache:       *ccqBackfillCache,
			TxVerifierEnabled:      slices.Contains(txVerifierChains, vaa.ChainIDTron),
		}
		watcherConfigs = append(watcherConfigs, wc)
	}
