...
func (w *Watcher) cclEnable(ctx context.Context) error {
	...
	_, err = CCLReadContract(ctx, w.tronConn.Client(), addrStr, ethCommon.Address{})
	if err != nil {
...
func (w *Watcher) cclReadContract(ctx context.Context, emitterAddr ethCommon.Address) ([32]byte, error) {
	...
	data, err := CCLReadContract(ctx, w.tronConn.Client(), w.cclAddr.Hex(), emitterAddr)
	if err != nil {
...
func CCLReadContract(ctx context.Context, tronClient *client.GrpcClient, cclAddr string, emitterAddr ethCommon.Address) ([32]byte, error) {
	return GetConfiguration(tronClient, emitterAddr, cclAddr)
}
