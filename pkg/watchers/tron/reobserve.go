...
func (w *Watcher) handleReobservationRequest(ctx context.Context, chainId vaa.ChainID, txID []byte, tronConn Connector, finalizedBlockNum, safeBlockNum uint64) (numObservations uint32, err error) {
	...
	_, blockNumber, msgs, err := MessageEventsForTransaction(timeout, tronConn, w.contract, w.chainID, tx)
	cancel()
