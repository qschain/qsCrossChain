...
var (
	tronConnectionErrors = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_tron_connection_errors_total",
			Help: "Total number of Tron connection errors (either during initial connection or while watching)",
		}, []string{"tron_network", "tron_reason"})

	tronMessagesObserved = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_tron_messages_observed_total",
			Help: "Total number of tron messages observed (pre-confirmation)",
		}, []string{"tron_network"})
	tronMessagesOrphaned = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_tron_messages_orphaned_total",
			Help: "Total number of tron messages dropped (orphaned)",
		}, []string{"tron_network", "tron_reason"})
	tronMessagesConfirmed = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_tron_messages_confirmed_total",
			Help: "Total number of tron messages verified (post-confirmation)",
		}, []string{"tron_network"})
	currentTronHeight = promauto.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "wormhole_tron_current_height",
			Help: "Current Tron block height",
		}, []string{"tron_network"})
	currentTronSafeHeight = promauto.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "wormhole_tron_current_safe_height",
			Help: "Current Tron safe block height",
		}, []string{"tron_network"})
	currentTronFinalizedHeight = promauto.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "wormhole_tron_current_finalized_height",
			Help: "Current Tron finalized block height",
		}, []string{"tron_network"})
	queryTronLatency = promauto.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "wormhole_tron_query_latency",
			Help: "Latency histogram for Tron calls (note that most interactions are streaming queries, NOT calls, and we cannot measure latency for those",
		}, []string{"tron_network", "operation"})
)
...
func (w *Watcher) Run(parentCtx context.Context) error {
	...
	// Connect to the node using the appropriate type of connector.
	{
		...
		w.tronConn, finalizedPollingSupported, safePollingSupported, err = w.createConnector(timeout, w.url)
		cancel()
		if err != nil {
			tronConnectionErrors.WithLabelValues(w.networkName, "dial_error").Inc()
	...
	if err := w.fetchAndUpdateGuardianSet(logger, ctx, w.tronConn); err != nil {
	...
	common.RunWithScissors(ctx, errC, "evm_fetch_messages", func(ctx context.Context) error {
		for {
			...
			case ev := <-messageC:
				...
				if err != nil {
					tronConnectionErrors.WithLabelValues(w.networkName, "block_by_number_error").Inc()
		...
		for {
			...
			case ev := <-headSink:
				...
				case Latest:
					...
					currentTronHeight.WithLabelValues(w.networkName).Set(float64(blockNumberU))
				...
				case Safe:
					...
					currentTronSafeHeight.WithLabelValues(w.networkName).Set(float64(blockNumberU))
				...
				case Finalized:
					...
					currentTronFinalizedHeight.WithLabelValues(w.networkName).Set(float64(blockNumberU))
				...
				for key, pLock := range w.pending {
					...
					tx, err := w.tronConn.TransactionReceipt(timeout, eth_common.BytesToHash(pLock.message.TxID))
					queryTronLatency.WithLabelValues(w.networkName, "transaction_receipt").Observe(time.Since(msm).Seconds())
					cancel()
					...
					if tx == nil || errors.Is(err, rpc.ErrNoResult) || (err != nil && err.Error() == "not found") {
						...
						tronMessagesOrphaned.WithLabelValues(w.networkName, "not_found").Inc()
						continue
					}
					...
					if tx.Result != 0 {
						logger.Error("transaction receipt with non-success status",
							...
							zap.Error(err))
						delete(w.pending, key)
						tronMessagesOrphaned.WithLabelValues(w.networkName, "tx_failed").Inc()
						continue
					}
					...
					if err != nil {
						if pLock.height+MaxWaitConfirmations <= blockNumberU {
							...
							tronMessagesOrphaned.WithLabelValues(w.networkName, "timeout").Inc()
							delete(w.pending, key)
						} else {
					...
					txHash := eth_common.BytesToHash(pLock.message.TxID)
...
func (w *Watcher) fetchAndUpdateGuardianSet(
	...
	if err != nil {
		tronConnectionErrors.WithLabelValues(w.networkName, "guardian_set_fetch_error").Inc()
		p2p.DefaultRegistry.AddErrorCount(w.chainID, 1)
		return err
	}

	queryTronLatency.WithLabelValues(w.networkName, "get_guardian_set").Observe(time.Since(msm).Seconds())
...
func fetchCurrentGuardianSet(ctx context.Context, tronConn Connector) (uint32, *StructsGuardianSet, error) {
	currentIndex, err := tronConn.GetCurrentGuardianSetIndex(ctx)
	if err != nil {
		return 0, nil, fmt.Errorf("error requesting current guardian set index: %w", err)
	}

	gs, err := tronConn.GetGuardianSet(ctx, currentIndex)
	if err != nil {
...
func (w *Watcher) getBlockTime(ctx context.Context, blockHash eth_common.Hash) (uint64, error) {
	msm := time.Now()
	timeout, cancel := context.WithTimeout(ctx, 15*time.Second)
	blockTime, err := w.tronConn.TimeOfBlockByHash(timeout, blockHash)
	cancel()
	queryTronLatency.WithLabelValues(w.networkName, "block_by_number").Observe(time.Since(msm).Seconds())
	return blockTime, err
}
func (w *Watcher) getBlockTimeByNum(ctx context.Context, blockNum int64) (uint64, error) {
	block, err := w.tronConn.Client().GetBlockByNum(blockNum)
	if err != nil {
		fmt.Println(err)
		return 0, err
	}
	return uint64(block.BlockHeader.RawData.Timestamp), nil
}
...
func (w *Watcher) postMessage(
	...
	ev *AbiLogMessagePublished,
	blockTime uint64,
) {
	fmt.Println("######ev.sender######")
	fmt.Println(ev.Sender)
	...
	tronMessagesObserved.WithLabelValues(w.networkName).Inc()
...
func (w *Watcher) verifyAndPublish(
	...
	tronMessagesConfirmed.WithLabelValues(w.networkName).Inc()
...
func (w *Watcher) waitForBlockTime(ctx context.Context, logger *zap.Logger, errC chan error, ev *AbiLogMessagePublished) {
...
func (w *Watcher) logVersion(ctx context.Context, logger *zap.Logger) {
	...
	logger.Info("node version",
		zap.String("version", "version_tron"),
		zap.String("network", w.networkName),
	)
}

// msgIdFromLogEvent formats the message ID (chain/emitterAddress/seqNo) from a log event.
func msgIdFromLogEvent(chainID vaa.ChainID, ev *AbiLogMessagePublished) string {
	return fmt.Sprintf("%v/%v/%v", uint16(chainID), evm.PadAddress(ev.Sender), ev.Sequence)
}

// createConnector determines the type of connector needed for a chain and creates the appropriate one.
func (w *Watcher) createConnector(ctx context.Context, url string) (tronConn Connector, finalizedPollingSupported, safePollingSupported bool, err error) {
	finalizedPollingSupported, safePollingSupported, err = w.getFinality(ctx)
	if err != nil {
		err = fmt.Errorf("failed to determine finality: %w", err)
		return
	}
	baseConnector, err := NewTronBaseConnector(w.networkName, url, w.contract, w.logger)
	if err != nil {
		err = fmt.Errorf("dialing tron client failed: %w", err)
		return
	}
	...
	if finalizedPollingSupported {
		...
		tronConn = NewInstantFinalityConnector(baseConnector, w.logger)
	} else {
		tronConn = NewInstantFinalityConnector(baseConnector, w.logger)
	}
	return
